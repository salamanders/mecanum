...
## Overview
This document describes the design for the robot's "Personality" system, which uses a permanently mounted Android phone to act as the robot's face, sensor package, and autonomous brain.

The system adds expressiveness via an animated eye display and enables autonomous "chasing" behavior using web-based computer vision, while maintaining a safety override for manual control.
...
2.  **The Sensor:** Reads device IMU data (DeviceMotion/DeviceOrientation) for compass/gyro headers (replacing `sensor.html`).
3.  **The Vision System:** Runs a local JavaScript Object Detection model (TensorFlow.js / COCO-SSD) to detect people or objects.

### 2. Autonomous Logic (Client-Side)
- **Object Detection:** The phone captures video frames and processes them locally using TF.js.
- **Target Selection:** The logic selects the "most interesting" object (e.g., the largest "person" or a specific color).
